...
for job in `echo $JOBS`; do
  rsync -auvz -L -e "ssh -i $HOME/.ssh/JENKINS/id_rsa" root@$HOST:/var/lib/jenkins/jobs/$job $TMP/ &>> $LOG
done
echo "**** END LOCAL SYNC ****"  >> $LOG
echo "**** BEGIN REMOTE SYNC ****"  >> $LOG
for job in `echo $JOBS`; do
  rsync -auvz -L -e "ssh -i $HOME/.ssh/JENKINS/id_rsa" $TMP/$job 524ee18d4382ec1886000084@$REMOTE:~/app-root/data/jobs/ &>> $LOG
done
